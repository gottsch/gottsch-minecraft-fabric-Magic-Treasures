...
public class NightVisionSpell extends CooldownSpell {
	...
	public SpellResult execute(World world, Random random, ICoords coords, ICastSpellContext context) {
		...
		if (manaComponent.mana() > 0 && context.getPlayer().isAlive()) {
			if (!context.getPlayer().hasStatusEffect(StatusEffects.NIGHT_VISION)) {
				context.getPlayer().addStatusEffect(new StatusEffectInstance(StatusEffects.NIGHT_VISION, spellFactorsComponent.modifyDuration(getDuration()), getAmplifier()));
			}
	...
	public Text getSpellDesc() {
		return Text.translatable(LangUtil.tooltip("spell.night_vision.rate"),
