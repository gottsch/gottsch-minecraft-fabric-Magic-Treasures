...
public class SpeedSpell extends CooldownSpell {
	...
	public SpellResult execute(World world, Random random, ICoords coords, ICastSpellContext context) {
		...
		if (manaComponent.mana() > 0 && context.getPlayer().isAlive()) {
			if (!context.getPlayer().hasStatusEffect(StatusEffects.SPEED)) {
				context.getPlayer().addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, spellFactorsComponent.modifyDuration(getDuration()), getAmplifier()));
			}
