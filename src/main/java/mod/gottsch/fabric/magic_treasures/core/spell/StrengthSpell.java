...
public class StrengthSpell extends CooldownSpell {
	...
	public SpellResult execute(World world, Random random, ICoords coords, ICastSpellContext context) {
		...
		if (manaComponent.mana() > 0 && context.getPlayer().isAlive()) {
			if (!context.getPlayer().hasStatusEffect(StatusEffects.STRENGTH)) {
				context.getPlayer().addStatusEffect(new StatusEffectInstance(StatusEffects.STRENGTH, (int)modifyDuration(context.getJewelry()), getAmplifier()));
			}
	...
	public Text getSpellDesc() {
		return Text.translatable(LangUtil.tooltip("spell.strength.rate"),
